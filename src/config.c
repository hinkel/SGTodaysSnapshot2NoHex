...
void readEEPROM(void)
{
    ...
    for (i = 0; i < 11; i++)
    {
        ...
        lookupThrottleRC[i] = cfg.esc_min + (int32_t) (cfg.esc_max - cfg.esc_min) * lookupThrottleRC[i] / 1000;     // [0;1000] -> [esc_min;esc_max]
    }
...
//  cfg.mag_dec                   = 0;          // For example, -6deg 37min, = -637 Japan, format is [sign]dddmm (degreesminutes) default is zero.
    ...
    cfg.acc_lpf                   = 100;	      // changed 27.11.2012
    cfg.acc_ilpf                  = 10;         // General LPF for all INS stuff

    cfg.looptime                  = 3000;	      // changed 27.11.2012 //    cfg.acc_lpf = 4;
    ...
    cfg.gy_smrll                  = 0;
    cfg.gy_smptc                  = 0;
    cfg.gy_smyw                   = 0;          // Ensure at least 3 in Tricoptermode for yaw
    ...
    cfg.as_trgt                   = 2;          // [0 - 15m  DEFAULT:0 (0 = Disable)] Autostart Targethight in m Note: use 3m or more
    ...
    cfg.rc_auxch                  = 4;          // [4 - 10] cGiesen: Default = 4, then like the standard!
...
//  cfg.esc_nfly                  = 0;          // This is the absolute throttle that kicks off the "has landed timer" if it is too low cfg.rc_min + 5% is taken.
    cfg.esc_pwm                   = 400;
    cfg.srv_pwm                   = 50;
