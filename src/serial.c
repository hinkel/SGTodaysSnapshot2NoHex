     ...
     if (c_state == HEADER_START)
     {
      ...
      if (c_state == HEADER_M)
      {
       ...
       if (c_state == HEADER_ARROW)
       {
           ...
           checksum ^= c;
           c_state   = HEADER_SIZE;      // the command is to follow // guiConnected = true;
       }
       else
        if (c_state == HEADER_SIZE)
        {
            cmdMSP    = c;
            checksum ^= c;
            c_state   = HEADER_CMD;
         ...
         if (c_state == HEADER_CMD && offset < dataSize)
         {
             checksum       ^= c;
             inBuf[offset++] = c;
         }
